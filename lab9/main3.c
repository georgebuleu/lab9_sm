...
int main() {
    ...
    *(int *)BARRIER_2_ADDR = 0;
    ...
    XIntc_Initialize(&intc_3, XPAR_AXI_INTC_3_DEVICE_ID);
    XIntc_Connect(&intc_3,
   			   XPAR_INTC_0_TMRCTR_0_VEC_ID,
   			   (XInterruptHandler)timer_0_isr,
   			   (void *)0);
    XIntc_Start(&intc_3, XIN_REAL_MODE);
    XIntc_Enable(&intc_3,
   		 XPAR_INTC_0_TMRCTR_0_VEC_ID);
    // ---------------------------------------------------------------------------------------
    ...
    // 1 int = 4 chars
    for(i = 12; i < DDR2_10_MB_SIZE; i = i + 16) {
   	 *(int *)(DDR2_BASE_ADDRESS + i) = 100000000 - (XTmrCtr_GetValue(&timer_0, 0) % 100000);
    }
    ...
    sort((int *)(DDR2_BASE_ADDRESS + 3 * DDR2_10_MB_SIZE / 4), 0, DDR2_10_MB_SIZE / 4 / sizeof(int) - 1);
