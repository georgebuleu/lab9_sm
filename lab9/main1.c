...
int main() {
    ...
    *(int *)BARRIER_2_ADDR = 0;
    ...
    XIntc_Initialize(&intc_1, XPAR_AXI_INTC_1_DEVICE_ID);
    XIntc_Connect(&intc_1,
   			   XPAR_INTC_0_TMRCTR_0_VEC_ID,
   			   (XInterruptHandler)timer_0_isr,
   			   (void *)0);
    XIntc_Start(&intc_1, XIN_REAL_MODE);
    XIntc_Enable(&intc_1,
   		 XPAR_INTC_0_TMRCTR_0_VEC_ID);
    // ---------------------------------------------------------------------------------------
    ...
    // 1 int = 4 chars
    for(i = 4; i < DDR2_10_MB_SIZE; i = i + 16) {
   	 *(int *)(DDR2_BASE_ADDRESS + i) = XTmrCtr_GetValue(&timer_0, 0) % 100000;
    }
    ...
    sort((int *)(DDR2_BASE_ADDRESS + DDR2_10_MB_SIZE / 2), DDR2_10_MB_SIZE / (2 * sizeof(int)), DDR2_10_MB_SIZE / sizeof(int) - 1);
